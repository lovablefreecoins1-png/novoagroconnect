    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {anuncios.map(a => (
        <Link key={a.id} to={`/marketplace/${a.id}`} className="card-agro !p-0 overflow-hidden hover:border-primary/30 transition-colors">
          {a.fotos && a.fotos.length > 0 ? <img src={a.fotos[0]} alt={a.titulo} className="w-full h-36 object-cover" /> : <div className="w-full h-36 bg-muted flex items-center justify-center"><Store size={24} className="text-muted-foreground/40" /></div>}
          <div className="p-3">
